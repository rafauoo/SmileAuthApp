...
def test_comment_get_content():
    comment = Comment(
        0, 2, {"en": "English comment", "pl": "Polski komentarz"}
    )
    assert comment.get_content() == {
        "en": "English comment",
        "pl": "Polski komentarz",
    }
...
def test_comment_repr():
    ...
    assert (
        repr(comment)
        == "Comment(min=0, max=2, content={'en': 'English comment'})"
    )
        ...
        [
            {
                "min": 0,
                "max": 2,
                "content": {"en": "Test comment 1", "pl": "Komentarz 1"},
            },
            {
                "min": 3,
                "max": 5,
                "content": {"en": "Test comment 2", "pl": "Komentarz 2"},
            },
        ]
    )
